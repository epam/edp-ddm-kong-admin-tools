...
| `config.allow_token_auth` | false | false | Allows access with Bearer token. If enabled introspection endpoint has to be set |
...
| `config.unauth_action` | deny | true | Action if user not authenticated:nil - redirect to auth, pass - do not redirect, deny - do not redirect but return an error instead |
| `config.access_token_header_name` | X-Access-Token | true | Name for header with access token |
| `config.bearer_access_token` | "no" | true | Use bearer access token |
| `config.id_token_header_name` | X-ID-Token | true | Name for header with ID token |
| `config.user_header_name` | X-Userinfo | true | Name for header with user info |
| `config.session_opts` | | false | JSON configuration string for [lua-resty-session](https://github.com/bungle/lua-resty-session) |
| `config.enable_authorization` | true | false | Enable user authorization based on JWT token issuer, scope and roles |
| `config.issuers_allowed` | | false | List of allowed access token issuers (iss claim) in the following format: issuers_allowed:\["https://domain/issuer1", "https://domain/issuer2"] |
| `config.scopes_required` | nil | false | The scopes required to be present in access token for successful authorization |
| `config.roles_required` | nil | false | The roles (roles_claim claim) required to be present in the access token (at least one from the list) for successful authorization |
| `config.realm_roles_required` | nil | false | The realm roles (realm_access.roles claim) required to be present in the access token (at least one from the list) for successful authorization |
| `config.client_roles_required` | nil | false | The realm roles (resource_access claim) required to be present in the access token (at least one from the list) for successful authorization |
| `config.token_claim_header_value` | | false | Access token claim name whose values will be set to the custom "token-claim" request header |
...

{
    ...
    "config": {
        ...
        "session_opts": "{\"cookie\" : { \"samesite\" : \"None\", \"secure\":true }}"
    },
